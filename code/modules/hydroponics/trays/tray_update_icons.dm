	...
	if(!isnull(seed))
		...
		if(dead)
			...
			if(!dead_overlay)
				...
				if(!seed.get_trait(TRAIT_PLANT_ICON_OVERRIDE)) // ignore coloring the sprite if the icon is overridden
			...
			if(seed.get_trait(TRAIT_PLANT_ICON_OVERRIDE)) // overridden icon drawing, just draw the icon from hydroponics_growing, dont do any bullshit to it
				if(harvest && overlay_stage == seed.growth_stages)
					ikey = "[seed.get_trait(TRAIT_PLANT_ICON)]-harvest"
				plant_overlay = image('icons/obj/hydroponics_growing.dmi', "[ikey]")
				add_overlay(plant_overlay)

			else // normal icon drawing behavior, takes a bunch of seperate sprites, colors them, and merges them into one
