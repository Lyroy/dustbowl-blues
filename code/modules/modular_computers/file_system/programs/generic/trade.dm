...
/datum/computer_file/program/trade/Topic(href, href_list)
	...
	if(href_list["PRG_build_order"])
		...
		if(account != department_accounts[FACTION_WAYWARD])
			orders_locked = TRUE
	...
	if(href_list["PRG_page_select"])
		...
		switch(href_list["PRG_page_select"])
			...
			if(LOG_SCREEN)
				input = input("Enter page number (1-[log_page_max])", "Page Selection") as num|null
				if(!input)
					return
...
/datum/nano_module/program/trade/nano_ui_data()
	...
	if(PRG.account)
		...
		if(dept_id)
			is_all_access = (dept_id == FACTION_WAYWARD) ? TRUE : FALSE
	...
	if(PRG.prg_screen == PRG_MAIN)
		...
		if(PRG.trade_screen == LOG_SCREEN)
			...
			.["log_page_max"] = PRG.log_page_max ? PRG.log_page_max : 1
