...
/datum/nano_module/program/card_mod/nano_ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = NANOUI_FOCUS, var/datum/nano_topic_state/state = GLOB.default_state)
	...
	data["wayward_jobs"] = format_jobs(wayward_positions)
	data["skymarshal_jobs"] = format_jobs(skymarshal_positions)
	data["followers_jobs"] = format_jobs(followers_positions)
	data["tribe_jobs"] = format_jobs(tribe_positions)
	data["wastelander_jobs"] = format_jobs(wastelander_positions)
	data["raider_jobs"] = format_jobs(raider_positions)
	data["bos_jobs"] = format_jobs(bos_positions)
