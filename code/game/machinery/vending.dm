...
/obj/machinery/vending
	...
	*/
	...
	var/vendor_department = FACTION_WAYWARD  //If set, members can manage this vendomat. earnings_account is set to the department's account automatically.
		...
		if(QUALITY_WELDING)
			if(custom_vendor)
				if(!panel_open)
					...
					return
				if(I.use_tool(user, src, WORKTIME_EXTREMELY_LONG, tool_type, FAILCHANCE_NORMAL, required_stat = SKILL_REP))
